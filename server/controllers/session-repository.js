...
const createSessionFromNotification = async (
  notificationId,
  sessionType,
  sessionInfo
) => {
...
const getDailySessions = async () => {
  try {
    ...
    return {
      result: true,
      sessions: [...faceSessions, ...bodySessions, ...laserSessions],
    };
  } catch (error) {
    return { result: false, message: error.message };
  }
...
const getClientSessions = async (clientId) => {
  try {
    const faceSessions = await FaceSession.find({ client: clientId }).populate(
      "client"
    );
    const bodySessions = await BodySession.find({ client: clientId }).populate(
      "client"
    );
    const laserSessions = await LaserSession.find({
      client: clientId,
...
const filterSessionsByDate = async (startDate, endDate) => {
  try {
    ...
    const endDate1 = new Date(
      Date.UTC(then.getFullYear(), then.getMonth(), then.getDate(), 23, 59, 59)
    );
    const faceSessions = await FaceSession.find({
...
const findSession = async (sessionId) => {
  try {
    const faceSession = await FaceSession.findById(sessionId).populate(
      "client"
    );
    const bodySession = await BodySession.findById(sessionId).populate(
      "client"
    );
    const laserSession = await LaserSession.findById(sessionId).populate(
      "client"
    );
...
const getFinishedSessions = async (startDate, endDate) => {
  try {
    ...
    const finishedSessions = await Session.find({
      date: { $gte: startDate1, $lte: endDate1 },
    }).populate("client");

    const totalProfit = finishedSessions.reduce((sum, session) => {
